...
def search_fasta(protein_id, fasta_file):
    ...
    for record in SeqIO.parse(fasta_file, 'fasta'):
        fasta_id = re.sub(r'^[^:]+:', '', record.id)  
        if fasta_id == protein_id:
            matched_seqs.append(str(record.seq))
    return matched_seqs
